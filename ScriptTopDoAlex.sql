...
create view projetos_ativos	as select p.nome, p.descricao from projetos p 
...
delimiter ;
call inserir_funcionario ('Enzo', 'enzo@email.com', '12345667', 1, 40.00);
	
	
	
delimiter //
	create procedure calcular_valor_total_projeto ()
	begin 
		declare horas_trabalhadas int;
		declare valor_hora int;
		select valor_hora, horas_trabalhadas from funcionarios_projetos fp 
		set horas_trabalhadas = fp.horas_trabalhadas;
		set valor_hora = fp.valor_hora;
		set valor_total = hora_trabalhadas * valor_hora;
	end// 
delimiter ;
call calcular_valor_total_projeto;
